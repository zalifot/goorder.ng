...
import { BadgeCheck, Clock, Receipt, TrendingUp } from 'lucide-react';
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">Payment transactions from all your shops.</p>
                </div>
                                        ...
                                        <div className="flex items-center gap-3 py-3">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    {tx.customer_name} · {formatDate(tx.created_at)}
                                                </p>
                                                <p className="text-xs text-muted-foreground">
                                                    {tx.shop ? (
                                                        <Link href={`/vendor/manage/shop/${tx.shop.public_id}`} className="hover:underline">
                                                            {tx.shop.name}
                                                        </Link>
                                                    ) : '—'} · <span className={`font-medium capitalize ${getStatusColor(tx.status)}`}>{tx.status}</span>
                                                </p>
                                            </div>
